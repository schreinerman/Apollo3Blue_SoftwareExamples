 ...
 ** and CTIMER A3 for generating triggers for ADC scan
 **
 ** History:
...
int main(void)
{
    ...
    u32Cfg =      _VAL2FLD(ADC_CFG_CKMODE,ADC_CFG_CKMODE_LLCKMODE)   //Low Latency Clock Mode
               ...
               |  _VAL2FLD(ADC_CFG_RPTEN,ADC_CFG_RPTEN_REPEATING_SCAN)  //Repeated scan
    ...
    ADC->INTEN_b.SCNCMP = 0;              //disable ADC scan complete interrupt - triggered by timer
    ...
    CTIMER->CTRL3_b.TMRA3CLR = 1;                                   //Clear timer
    CTIMER->CTRL3_b.TMRA3CLK = CTIMER_CTRL3_TMRA3CLK_HFRC_DIV1024;  //Setup timer for HFRC / 1024
    CTIMER->CTRL3_b.TMRA3FN = CTIMER_CTRL3_TMRA3FN_REPEATEDCOUNT;   //Repeated count mode
    CTIMER->CTRL3_b.ADCEN = 1;

    SystemCoreClockUpdate();  //CMSIS-API: update SystemCoreClock variable
    //
    // Scan conversion every second:
    ...
    //             +----------------+              |     |
    //                              |              |     |
    CTIMER->CMPRA3_b.CMPR0A3 = (SystemCoreClock / 1024 / 16) - 1; //Set compare value, so ADC trigger every second / average samples
    CTIMER->CTRL3_b.TMRA3CLR = 0;   //run mode
    CTIMER->CTRL3_b.TMRA3EN = 1;    //start timer
